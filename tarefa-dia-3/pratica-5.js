...
//tentei atribuir uma variavel para receber receita.length e não funciona, lembrar disso
let valorvendas = 0
let aux = 0
while(aux < receita.length)
{
    valorvendas += receita[aux].valor
    aux++
}

console.log(`o valor total da receita sem gorjeta: ${valorvendas}`)
/* ----------------------------------------------------------------------------- */
...
while(aux < receita.length)
{
    valorgorjeta += receita[aux].gorjeta
    aux++
}

console.log(`O valor total, incluindo as gorjetas é: ${valorgorjeta + valorvendas}`)
...
while(aux < receita.length)
{
    if((receita[aux].valor + receita[aux].gorjeta) < 50)
    {
        menor50++
        aux++
    }
...
console.log(`A quantidade de vendas menores que 50 reais são: ${menor50}`)
